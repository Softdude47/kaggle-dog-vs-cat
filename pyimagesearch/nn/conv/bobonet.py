...
class BoboNet:
    ...
    def build(input_shape, classes=2, reg=0.0002):
        ...
        unit1 = get_nearest_pow_of_2(input_shape)
...
def get_nearest_pow_of_2(num:"int | float"):
    next_pow_of_2 = pow(2, math.ceil(math.log(num)/math.log(2)))
    
    prev_pow_of_2 = next_pow_of_2 / 2
    
    nearest_pow = next_pow_of_2
    if abs(math.sqrt(num) - prev_pow_of_2) < abs(math.sqrt(num) - next_pow_of_2):
